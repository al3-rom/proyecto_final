    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col items-center justify-center overflow-hidden font-sans">
            ...
            <div className="relative z-10 w-full max-w-4xl px-4 flex flex-col items-center">
                <div className="mb-2 sm:mb-6 relative group cursor-default">
                    <div className="absolute inset-0 bg-emerald-500/20 blur-[40px] rounded-full opacity-60 group-hover:opacity-100 group-hover:bg-emerald-500/30 transition-all duration-700 ease-in-out scale-[1.3] pointer-events-none"></div>
                    <img
                        src={Logotipo}
                        alt="Econight Pass"
                        className="w-32 h-32 sm:w-40 sm:h-40 object-contain relative z-10 drop-shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:scale-110 hover:drop-shadow-[0_0_25px_rgba(16,185,129,0.6)] transition-all duration-500"
                    />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-6 w-full max-w-3xl lg:px-0">
                    {languages.map((lang) => (
                        <button
                            key={lang.code}
                            onClick={() => handleSelect(lang.code)}
                            className="group relative flex flex-col items-center justify-center py-3.5 px-6 sm:p-8 bg-[#0a0a0a] rounded-2xl sm:rounded-[2rem] border border-zinc-800 hover:border-emerald-500/50 transition-all duration-500 ease-out hover:-translate-y-1 sm:hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(16,185,129,0.2)] focus:outline-none"
                        >
                            <div className="absolute inset-0 rounded-2xl sm:rounded-[2rem] bg-emerald-500 opacity-0 group-hover:opacity-[0.03] transition-opacity duration-500"></div>

                            <span className={`fi ${lang.flagClass} text-[2rem] sm:text-5xl mb-1 sm:mb-4 group-hover:scale-110 transition-transform duration-300 drop-shadow-md rounded-sm leading-none`}></span>
                            <span className="text-base sm:text-xl font-bold text-white mb-0.5 sm:mb-1 tracking-wide">{lang.name}</span>
                            <span className="text-[10px] sm:text-sm font-medium text-emerald-400/80 bg-emerald-950/40 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full border border-emerald-500/20 mt-1 sm:mt-2 opacity-80 group-hover:opacity-100 transition-opacity">{lang.greeting}</span>
                        </button>
                    ))}
                </div>
            </div>
